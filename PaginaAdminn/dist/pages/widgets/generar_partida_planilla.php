<?php
// generar_partida_planilla.php
include 'conexion.php';

if (!isset($_GET['planilla_id'])) {
  die('Falta parámetro planilla_id');
}
$planilla_id = (int)$_GET['planilla_id'];
...
<head>
  ...
  <title>Generar Partida Contable (Planilla)</title>
  ...
  <style>
    ...
    .debe-input[readonly], .haber-input[readonly] { background-color:#d3d3d3!important; opacity:1; color:#666; }
    ...
    .amounts-column { width:120px; text-align:right; }
    .cuenta-column { width:300px; }
    .descripcion-totales { font-style:italic; color:#666; font-weight:bold; }
  </style>
</head>
<body class="p-4">
  <h1 class="mb-3">Partida Contable (Planilla #<?= $planilla_id ?>)</h1>
  <input type="hidden" id="planilla_id" value="<?= $planilla_id ?>">
  ...
  $(function(){
    ...
    function ordenSeccion(s){ return s==='activo'?1 : s==='pasivo'?2 : s==='patrimonio'?3 : 4; }

    function reordenarTabla() {
      let filas = [];
      $('#tbody-cuentas tr[data-id]').each(function(){
        filas.push({ orden: ordenSeccion($(this).data('seccion')), el: $(this).clone(true) });
        $(this).remove();
      });
    ...
    function manejarCambio(sel, tipo) {
      $('#'+sel).on('change', function(){
        ...
        $('#'+sel+' option').each(function(){
          let id=this.value, existe=vals.includes(id), fila=$(`tr[data-id="${id}"][data-tipo="${tipo}"]`);
          if (existe && !fila.length) {
            let nombre=$(this).data('nombre'), clasif=$(this).data('clasificacion'), sec=seccionPara(clasif);
            let row = $(`
            ...
            `);
            $('#tbody-cuentas').append(row); reordenarTabla();
          }
    ...
    function recalcular(){
      let td=0, th=0;
      $('.debe-input').each(function(){ td += parseFloat($(this).val())||0; });
      $('.haber-input').each(function(){ th += parseFloat($(this).val())||0; });
      $('#total-debe').text(td.toFixed(2));
      $('#total-haber').text(th.toFixed(2));
    }
    $('#descripcion').on('input', function(){
      const t = $(this).val().trim();
      $('#descripcion-final').text(t==='' ? 'Descripción de la partida:' : t+':');
    });
    $(document).on('input change keyup','.debe-input,.haber-input',recalcular);
    recalcular();

    $('#btnGuardar').on('click', function(){
      let td = parseFloat($('#total-debe').text()), th = parseFloat($('#total-haber').text());
      if (td !== th) return Swal.fire({icon:'error',title:'Error',text:'Debe debe igualar a Haber.'});
      ...
      $('tr[data-id]').each(function(){
        ...
        if (debe>0 || haber>0) {
          detalles.push({ cuenta_id: $(this).data('id'), debe: debe, haber: haber });
        }
      });
      if (detalles.length===0) return Swal.fire({icon:'error',title:'Error',text:'Debe agregar al menos una cuenta con monto.'});

      $.ajax({
        url: 'guardar_partida_planilla.php',
        method: 'POST',
        dataType:'json',
        ...
        Swal.fire({icon:'success',title:'Guardado',timer:1200,showConfirmButton:false})
          .then(()=>{
            // Ajusta el nombre del export si usas otro archivo:
            $('#exportBtnContainer').html(
              `<a href="exportar_partida_planilla_pdf.php?partida_id=${res.partida_id}&planilla_id=${$('#planilla_id').val()}"
