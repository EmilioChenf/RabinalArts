<?php
// generar_Partida_contable_ventas.php
include 'conexion.php';

if (!isset($_GET['compra_id'])) {
  die('Falta parámetro compra_id');
}
...
<head>
  <meta charset="UTF-8">  
  <title>Generar Partida Contable de Compra</title>
...
<body class="p-4">
  <h1 class="mb-3">Partida Contable (Compra #<?= $compra_id ?>)</h1>
  <input type="hidden" id="compra_id" value="<?= $compra_id ?>">
  ...
  $(function(){
    ...
    $('#btnGuardar').on('click', function(){
      ...
      $.ajax({
        url: 'guardar_partida_compras.php',
        method: 'POST',
        dataType:'json',
        ...
        Swal.fire({icon:'success',title:'Guardado',timer:1200,showConfirmButton:false})
          .then(()=>{
            $('#exportBtnContainer').html(
              `<a href="exportar_compra_interna_pdf.php?partida_id=${res.partida_id}&compra_id=${$('#compra_id').val()}"
