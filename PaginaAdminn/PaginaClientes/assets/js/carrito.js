...
// ✅ Función para calcular el total correctamente con símbolo de dólar
function calcularTotal() {
    ...
    if (subtotalElement) subtotalElement.innerHTML = "$" + (total / 1.18).toFixed(2);
    if (igvElement) igvElement.innerHTML = "$" + (total * 0.18).toFixed(2);
    ...
    function leerLocalStorageCompra() {
        ...
        productosLS.forEach(producto => {
            ...
            row.innerHTML = `
                ...
                <td>$<span class="subtotal">${(producto.precio * producto.cantidad).toFixed(2)}</span></td>
                <td>
